set -e
#  Code generated by godownloader and customized for turbot's pkg.
#

usage() {
  this=$1
  cat <<EOF
$this: download binaries for turbot/cli
Usage: $this [-b] bindir [version]
  -b sets bindir or installation directory, default "/usr/local/bin"
   [version] is a version number from
   https://github.com/turbot/cli/releases
...
github_api() {
  ...
  echo "$local_file" "$source_url" "$header"
...
NAME=${BINARY}_cli_${VERSION}_${OS}_${ARCH}
TARBALL=${NAME}.${FORMAT}
TARBALL_URL=${GITHUB_DOWNLOAD}/${VERSION}/${TARBALL}
CHECKSUM=${BINARY}_cli_${VERSION}_checksums.txt
