...
@test "turbot graphql" {
  cd $TURBOT_MODS_DIR
  run turbot graphql --profile rocketeer --format "json" --query '{
	policyValue(uri: "tmod:@turbot/aws#/policy/types/turbotIamRoleExternalId", resourceId:"arn:aws:::650022101893"){
		value: secretValue
  }
}'
  [ "$status" -eq 0 ]
